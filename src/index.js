...
const STATIC_POSTFIX = `,"choices":${MOCK_CHOICES_JSON},"usage":${MOCK_USAGE_JSON}}`;
...
app.post('/v1/chat/completions', jsonParser, compressMiddleware, (req, res) => {
  ...
  const payload = `{"id":"chatcmpl-${crypto.randomUUID()}","object":"chat.completion","created":${Math.floor(Date.now() / 1000)},"model":${JSON.stringify(model)}${STATIC_POSTFIX}`;
