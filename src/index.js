...
let corsOptions = { origin: '*' }; // Default to wildcard to preserve backward compatibility

if (allowedOriginsStr) {
  const parsedOrigins = allowedOriginsStr.split(',').map(o => o.trim()).filter(Boolean);
  if (parsedOrigins.includes('*')) {
    corsOptions = { origin: '*' };
  } else {
    corsOptions = { origin: parsedOrigins };
  }
}
